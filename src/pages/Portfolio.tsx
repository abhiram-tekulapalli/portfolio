...
export default function Portfolio() {
  ...
  useEffect(() => {
    const loadState = async (url: string, setter: (data: any) => void) => {
      try {
    ...
    loadState('/api/v1/hero', setHero);
    loadState('/api/v1/about', setAbout);
    loadState('/api/v1/skills', setSkills);
    loadState('/api/v1/projects', setProjects);
    loadState('/api/v1/certifications', setCertifications);
    loadState('/api/v1/experience', setExperience);
    loadState('/api/v1/education', setEducation);
    loadState('/api/v1/blogs', setBlogs);
    loadState('/api/v1/settings', setSettings);
    loadState('/api/v1/integrations/leetcode', setLeetcode);
    loadState('/api/v1/integrations/github', setGithub);
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center justify-between text-[10px] font-mono uppercase tracking-wider text-text-muted">
                      <span>{c.issuer}</span>
                      <span>{c.dateIssued.substring(0, 7)}</span>
                    </div>
